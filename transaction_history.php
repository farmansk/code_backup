...
<script>
 ...
 function SEARCH(){
  ...
  form.action = "transaction_history.php?intNowPage=<?=$intNowPage?>";
  form.submit(); 
 ...
 function GO(val) {
  ...
  var form = document.Form ;
	...
	form.action = "transaction_history.php?intNowPage="+val;
	form.submit();
...
if ( window.innerWidth > 991 ) {
  ...
  document.writeln("<div id='right_box' style='margin-top:30px;background:#FFFFFF;width:100%;display:inline-block;border:2px #B0B1B3 solid;border-radius:10px;padding:30px'>");
   ...
   <div style="vertical-align:top;margin-left:-2px;display:inline-block;width:31%;text-align:center;color:#A61313;border-bottom:5px #A61313 solid;font-size:13pt;font-weight:500;">입출금내역<div style="height:10px"></div></div>
   
   
   <div style="margin-top:15px;text-align:left;font-size:11pt">※ Faunus Coin의 입출금 내역을 확인하실 수 있습니다.</div>
...
<?
  ...
  $Search_SQL = " WHERE UserEmail = '".$_SESSION["UserEmail"]."' AND target = 'token' "  ;
...
<?
  ...
  if($count > 0){
   ...
   foreach($query as $rs){
     ...
     }else if ( $rs["target"] == "BNB" ) {
       
      $s_bol = "BNB" ;
       
     }else if ( $rs["target"] == "cash" ) {
   ...
   ?>
                            ...
                            <tr>
                             ...
                             <th style="border-right:1px solid #E5E5E5;border-bottom:1px solid #E5E5E5;font-size:11pt;font-weight:500;padding:5px;text-align:right"><a href="javascript:popup('<?=$rs["from_address"]?>','<?=$rs["amount"]?>','<?=$s_bol?>')" style="color:#262626"><?=substr($rs["from_address"],0,5)?></a></th>
                            </tr>
