...
$query = mysqli_query($link, "Select Count(seqno) from users Where UserEmail = '".$UserEmail."' and UserPwd = '".$UserPwd."'");
...
if ( $Count == 0 ) {
 ...
 $query = mysqli_query($link, "Select * from users Where UserEmail = '".$UserEmail."' and UserPwd = '".$UserPwd."'");
 $rs=$query->fetch_assoc();
 ...
 $query = mysqli_query($link, "select UserEmail from users Where UserEmail = '".$UserEmail."'") ;
 ...
 if ( $YN == "Y" ) {
  ...
  $query = mysqli_query($link, "select max(num) from login_connect Where UserEmail = '".$UserEmail."'") ;
  $count = mysqli_num_rows($query);
 ...
 $to_name = base64_decode($UserEmail) ;
 $to = base64_decode($UserEmail) ;
 ...
 if($count > 0){
  ...
  $data = mysqli_query($link,$SQL);
    ...
    $_SESSION['TempUserEmail'] = $UserEmail ;
