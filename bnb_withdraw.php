...
if ( window.innerWidth > 991 ) {
  ...
  document.writeln("<div id='right_box' style='margin-top:30px;background:#FFFFFF;width:100%;display:inline-block;border:2px #B0B1B3 solid;border-radius:10px;padding:30px'>");
   ...
   <div style="width:100%;display:inline-block;text-align:left;font-size:15pt;color:#A61313;font-weight:600"><img src="./img/line_bar.png" style="width:10px;height:20px;margin-right:8px"><?=$CompanyName?> BNB 입출금</div>
   ...
   <div style="margin-top:15px;margin-bottom:15px;border:1px #E5E5E5 solid;border-radius:10px;background:#FFF4F4;text-align:left">
    ...
    <div style="width:75%;display:inline-block;padding:10px;text-align:right"><span id="bnb_total_balance_str" style="font-weight:600">0</span>&nbsp;BNB</div>
   </div>
   
   <div style="vertical-align:top;display:inline-block;width:31%;text-align:center;color:#666666;border-bottom:2px #E5E5E5 solid;font-size:13pt;font-weight:500;cursor:pointer";cursor:pointer" onclick="location.href='./bnb_deposit.php'">입금하기<div style="height:13px"></div></div>
   ...
   <div style="display:inline-block;width:45%;text-align:right;font-size:11pt"><span id="bnb_total_balance_str2" style="font-weight:600">0</span>&nbsp;BNB</div>
   ...
   <div style="vertical-align:top;padding:5px;display:inline-block;width:48%;text-align:center;background:#A61313;color:#FFFFFF;border-top:1px #A61313 solid;border-left:1px #A61313 solid;border-right:1px #E5E5E5 solid;border-bottom:2px #E5E5E5 solid;font-size:13pt;font-weight:500;cursor:pointer" onclick="location.href='./bnb_withdraw.php'">내부전송</div>
   <div style="vertical-align:top;padding:5px;margin-left:-4px;display:inline-block;width:48%;text-align:center;color:#666666;border-top:1px #E5E5E5 solid;border-right:1px #E5E5E5 solid;border-bottom:2px #E5E5E5 solid;font-size:13pt;font-weight:500;cursor:pointer" onclick="location.href='./bnb_withdraw_out.php'">외부전송</div>    
   ...
   <div class="col-lg-12" style="text-align:left;margin-top:-15px">
    <div class="checkout__input mt-30">
     ...
     <input id="amount" name="amount" type="text" placeholder="최소 출금 수량 <?=$min_withdraw_bnb?> BNB   " style="text-align:right;display:inline-block;width:80%;" autocomplete="off" onblur="CALC()">
   ...
   <div style="display:inline-block;width:45%;text-align:right;font-size:11pt"><span id="withdraw_bnb_str" style="font-weight:600">0</span>&nbsp;BNB</div>
...
<script>
 ...
 async function WITHDRAW_PROCESS(){
     ...
     <?if ( $mobile_certification != "Y" ) {?>
      ...
      Swal.fire({
       icon: "warning",
       text: "BNB 입출금을 위해 휴대전화 본인 인증이 필요합니다.",
      }).then((ok) => {
    ...
    if ( parseFloat(document.getElementById("amount").value) < parseFloat(<?=$min_withdraw_bnb?>) ) {
     ...
     Swal.fire({
      icon: "warning",
      text: "최소 출금수량은 '<?=$min_withdraw_bnb?>BNB' 입니다.",
     }).then((ok) => {
     ...
     return;
       ...
       fetch("./bnb_withdraw_process.php", requestOptions)
       ...
       .then((result) => {
         ...
         if ( result.replace(/^\s+|\s+$/gm,'') == "no_account" ) { 
          ...
          Swal.fire({
           icon: "warning",
           text: "출금하실 <?=$tokenSymbol?>가 부족합니다. 확인 후 다시 시도해 주세요. ",
          }).then((ok) => {
