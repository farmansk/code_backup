...
if ( window.innerWidth > 991 ) {
  ...
  document.writeln("<div id='right_box' style='margin-top:30px;background:#FFFFFF;width:100%;display:inline-block;border:2px #B0B1B3 solid;border-radius:10px;padding:30px'>");
   ...
   <div style="vertical-align:top;display:inline-block;width:31%;text-align:center;color:#666666;border-bottom:2px #E5E5E5 solid;font-size:13pt;font-weight:500;cursor:pointer" onclick="location.href='./main.php'">입금하기<div style="height:13px"></div></div>
   <div style="vertical-align:top;margin-left:-2px;display:inline-block;width:31%;text-align:center;color:#A61313;border-bottom:5px #A61313 solid;font-size:13pt;font-weight:500;">출금하기<div style="height:10px"></div></div>
   ...
   <div style="display:inline-block;width:45%;text-align:right;font-size:11pt"><span id="coin_balance_str3" style="font-weight:600">0</span>&nbsp;<?=$Symbol?></div>
   ...
   <div style="vertical-align:top;padding:5px;display:inline-block;width:48%;text-align:center;background:#A61313;color:#FFFFFF;border-top:1px #A61313 solid;border-left:1px #A61313 solid;border-right:1px #E5E5E5 solid;border-bottom:2px #E5E5E5 solid;font-size:13pt;font-weight:500;cursor:pointer" onclick="location.href='./withdraw.php'">내부전송</div>
   <div style="vertical-align:top;padding:5px;margin-left:-4px;display:inline-block;width:48%;text-align:center;color:#666666;border-top:1px #E5E5E5 solid;border-right:1px #E5E5E5 solid;border-bottom:2px #E5E5E5 solid;font-size:13pt;font-weight:500;cursor:pointer" onclick="location.href='./withdraw_out.php'">외부전송</div>    
         
   <div class="col-lg-12" style="text-align:left">
    <div class="checkout__input mt-30">
     <p style="display:inline-block;margin-right:15px">출금주소</p>
     <input id="address" name="address" type="text" placeholder="출금할 주소 또는 이메일주소 입력" style="display:inline-block;width:80%;" autocomplete="off">
    </div>
   </div>     
   ...
   <div class="col-lg-12" style="text-align:left;margin-top:-15px">
    <div class="checkout__input mt-30">
     ...
     <input id="amount" name="amount" type="text" placeholder="최소 출금 수량 <?=$min_withdraw_token?> <?=$Symbol?>   " style="text-align:right;display:inline-block;width:80%;" autocomplete="off" onblur="CALC()">
   ...
   <div style="display:inline-block;width:45%;text-align:right;font-size:11pt"><span id="withdraw_coin_str" style="font-weight:600">0</span>&nbsp;<?=$Symbol?></div>
   
   <!--<div style="display:inline-block;width:45%;text-align:left;font-size:11pt">출금 수수료</div>
   <div style="display:inline-block;width:45%;text-align:right;font-size:11pt"><span id="withdraw_coin_str" style="font-weight:600"><?=$WithdrawFee?></span>&nbsp;BNB</div>-->
   ...
   <!--<div class="col-lg-12" style="text-align:left;margin-top:-15px">
    <div class="checkout__input mt-30">
     <p style="display:inline-block;margin-right:15px">메모</p>
     <input id="memo" name="memo" type="text" placeholder="" style="display:inline-block;width:90%;" autocomplete="off">
    </div>
   </div>  
...
<script>
 ...
 async function WITHDRAW_PROCESS(){
     ...
     <?if ( $mobile_certification != "Y" ) {?>
      ...
      Swal.fire({
       icon: "warning",
       text: "코인 입출금을 위해 휴대전화 본인 인증이 필요합니다.",
      }).then((ok) => {
    ...
    if ( parseFloat(document.getElementById("amount").value) < parseFloat(<?=$min_withdraw_token?>) ) {
      
     Swal.fire({
      icon: "warning",
      text: "최소 출금수량은 '<?=$min_withdraw_token?><?=$Symbol?>' 입니다.",
     }).then((ok) => {
     ...
     return;
       ...
       fetch("./withdraw_process.php", requestOptions)
...
async function CALC() {
 ...
 document.getElementById("withdraw_coin_str").innerHTML = document.getElementById("amount").value ;
...
async function PER(val) {
 ...
 if ( val == "10" ) {
  ...
  total_amount = parseFloat(coin_balance) * parseFloat(0.1) ;
  document.getElementById("amount").value = total_amount ;

  document.getElementById("withdraw_coin_str").innerHTML = total_amount;
  ...
  total_amount = parseFloat(coin_balance) * parseFloat(0.25) ;
  document.getElementById("amount").value = total_amount ;
  ...
  total_amount = parseFloat(coin_balance) * parseFloat(0.5) ;
  document.getElementById("amount").value = total_amount ;
  ...
  document.getElementById("amount").value = parseFloat(coin_balance) ;
  
  document.getElementById("withdraw_coin_str").innerHTML = parseFloat(coin_balance) ;
  ...
  .then((result) => {
       ...
       .then((result) => {
         ...
         .then((result) => {
          ...
          document.getElementById("coin_balance_str3").innerHTML = result;
