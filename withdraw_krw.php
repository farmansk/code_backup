...
if ( window.innerWidth > 991 ) {
  ...
  document.writeln("<div id='right_box' style='margin-top:30px;background:#FFFFFF;width:100%;display:inline-block;border:2px #B0B1B3 solid;border-radius:10px;padding:30px'>");
   ...
   <div style="vertical-align:top;margin-left:-2px;display:inline-block;width:31%;text-align:center;color:#A61313;border-bottom:5px #A61313 solid;font-size:13pt;font-weight:500;" onclick="location.href='./withdraw_krw.php'">출금신청<div style="height:10px"></div></div>
   ...
   <div class="col-lg-12" style="text-align:left">
    <div class="checkout__input mt-30">
     <p style="display:inline-block;margin-right:15px">입금 은행</p>
     <input id="depositor_bank_name" name="depositor_bank_name" type="text" placeholder="입금은행 입력" style="display:inline-block;width:80%;" autocomplete="off">
    </div>
   </div>     

   <div class="col-lg-12" style="text-align:left">
    <div class="checkout__input mt-30">
     <p style="display:inline-block;margin-right:15px">계좌 번호</p>
     <input id="depositor_bank_account" name="depositor_bank_account" type="text" placeholder="입금받을 계좌번호 입력" style="display:inline-block;width:80%;" autocomplete="off">
    </div>
   </div> 

   <div class="col-lg-12" style="text-align:left">
    <div class="checkout__input mt-30">
     <p style="display:inline-block;margin-right:15px">예금주</p>
     <input id="depositor_bank_account_holder" name="depositor_bank_account_holder" type="text" placeholder="입금받을 계좌 예금주 입력" style="display:inline-block;width:80%;" autocomplete="off">
    </div>
   </div> 
   
   <div class="col-lg-12" style="text-align:left;margin-top:-15px">
    <div class="checkout__input mt-30">
     <p style="display:inline-block;margin-right:15px">출금 금액</p>
     <input id="amount" name="amount" type="text" placeholder="최소 출금 금액 <?=number_format($min_withdraw_krw)?> KRW   " style="text-align:right;display:inline-block;width:80%;" autocomplete="off" onclick="CALC('A')" onblur="CALC('B')">
   ...
   <div style="display:inline-block;width:45%;text-align:left;font-size:11pt">출금수수료</div>
   <div style="display:inline-block;width:45%;text-align:right;font-size:11pt"><span style="font-weight:600"><?=$cash_withdraw_fee?></span>&nbsp;%</div>
   
   <div style="display:inline-block;width:45%;text-align:left;font-size:11pt">출금량</div>
   <div style="display:inline-block;width:45%;text-align:right;font-size:11pt"><span id="withdraw_cash_str" style="font-weight:600">0</span>&nbsp;KRW</div>
   
   <div style="display:inline-block;width:45%;text-align:left;font-size:11pt">수수료</div>
   <div style="display:inline-block;width:45%;text-align:right;font-size:11pt"><span id="withdraw_cash_fee_str" style="font-weight:600">0</span>&nbsp;KRW</div>
   ...
   <a href="javascript:DEPOSIT_PROCESS()" class="site-btn" style="margin-top:20px;width:100%;text-align:center;background:#A61313;font-size:12pt;border-radius:5px;">출금신청&nbsp;<img src="./img/lock_icon.png"></a>
...
async function CALC(val) {
 ...
 if ( document.getElementById("amount").value != "" ) {
  ...
  amount = numberWithCommas(document.getElementById("amount").value,"A") ;
  
  fee = parseInt( parseFloat(amount) * parseFloat(<?=$cash_withdraw_fee*0.01?>) ) ;
  ...
  document.getElementById("withdraw_cash_str").innerHTML = numberWithCommas(amount,"B") ;
  document.getElementById("withdraw_cash_fee_str").innerHTML = numberWithCommas(fee,"B") ;
...
async function PER(val) {
 ...
 if ( val == "10" ) {
  ...
  document.getElementById("per_box4").style.color = "#666666"; 
   ...
   amount = parseFloat(cash) * parseFloat(0.1) ;
  ...
  document.getElementById("per_box4").style.color = "#666666"; 
   ...
   amount = parseFloat(cash) * parseFloat(0.25) ;
  ...
  document.getElementById("per_box4").style.color = "#666666"; 
   ...
   amount = parseFloat(cash) * parseFloat(0.5) ;
 ...
 async function DEPOSIT_PROCESS(){
    ...
    if ( document.getElementById("depositor_bank_name").value == "" ) {
     ...
     Swal.fire({
      ...
      document.getElementById("depositor_bank_name").focus();
      return;
    ...
    if ( document.getElementById("depositor_bank_account").value == "" ) {
      
     Swal.fire({
      icon: "warning",
      text: "입금받을 계좌번호를 입력해 주세요.",
     }).then((ok) => {
       
      document.getElementById("depositor_bank_account").focus();
      return;
    ...
    if ( document.getElementById("depositor_bank_account_holder").value == "" ) {
      
     Swal.fire({
      icon: "warning",
      text: "입금받을 예금주를 입력해 주세요.",
     }).then((ok) => {
       
      document.getElementById("depositor_bank_account_holder").focus();
      return;
    ...
    if ( ( parseFloat(numberWithCommas(document.getElementById("amount").value,'A')) + parseFloat(fee) ) < parseFloat(<?=$min_withdraw_krw?>) ) {
      
     Swal.fire({
      icon: "warning",
      text: "최소 출금 금액은 '<?=number_format($min_withdraw_krw)?>KRW' 입니다.",
     }).then((ok) => {
     ...
     return;
       ...
       document.getElementById("loadingdiv_result").innerHTML = "출금신청 처리중입니다. 중간에 절대로 종료하지 마세요." ;
       
    
       var formdata = new FormData();
       

       formdata.append("depositor_bank_name", document.getElementById("depositor_bank_name").value);
       formdata.append("depositor_bank_account", document.getElementById("depositor_bank_account").value);
       formdata.append("depositor_bank_account_holder", document.getElementById("depositor_bank_account_holder").value);
       ...
       fetch("./withdraw_krw_process.php", requestOptions)
       ...
       .then((result) => {
         ...
         if ( result.replace(/^\s+|\s+$/gm,'') == "no_cash" ) { 
          ...
          Swal.fire({
           icon: "warning",
           text: "출금신청이 실패하였습니다. 잠시 후 다시 시도해 주세요. ",
          }).then((ok) => {
