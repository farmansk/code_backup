...
$query = mysqli_query($link, "Select Count(admin_id) from setting Where admin_id = '".$UserEmail."' and admin_pw = '".$UserPwd."'");
$rs=$query->fetch_assoc();
 
$Count = htmlspecialchars($rs["Count(admin_id)"]) ;

if ( $Count == 0 ) {



 $query = mysqli_query($link, "select admin_id from setting Where admin_id = '".$UserEmail."'") ;
 ...
 if ( $YN == "Y" ) {
  ...
  $query = mysqli_query($link, "select max(num) from admin_login_connect Where UserEmail = '".$UserEmail."'") ;
  $count = mysqli_num_rows($query);
  ...
  $SQL = "INSERT INTO admin_login_connect " ;
  $SQL = $SQL . " (" ;
  $SQL = $SQL . " num " ;
  $SQL = $SQL . " , UserEmail " ;
  $SQL = $SQL . " , regdate " ;
 ...
 $_SESSION['TempAdminEmail'] = $UserEmail;
